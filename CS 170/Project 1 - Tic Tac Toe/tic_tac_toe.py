...
class Player_2:
    ...
    def my_play(self):
        ...
        if len(possibleMoves) == 9:
            firstMovePos = random.choice([(0,0), (0,2), (2,0), (2,2)])
            self.board[firstMovePos[0]][firstMovePos[1]] = 'O'
            return
        elif len(possibleMoves) == 8 and self.board[1][1] == '-':
            self.board[1][1] = 'O'
            return
        # Minimax for AI
        minPoints = float('-inf')
        ...
        for i in range(3):
            for j in range(3):
                if self.board[i][j] == '-':
                    self.board[i][j] = 'O'
                    tempPoints = self.MiniMax(self.board, False, float('-inf'), float('inf'), possibleMoves)
                    self.board[i][j] = '-'
    ...
    def MiniMax(self, tempBoard, isMax, alpha, beta, possibleMoves):
        ...
        if isMax:
            ...
            for move in possibleMoves:
                ...
                tempBoard[i][j] = 'O'
                maxPoints = max(maxPoints, self.MiniMax(tempBoard, False, alpha, beta, possibleMoves))
            ...
            for move in possibleMoves:
                ...
                possibleMoves.remove(move)
                tempBoard[i][j] = 'X'
                minPoints = min(minPoints, self.MiniMax(tempBoard, True, alpha, beta, possibleMoves))
                possibleMoves.append(move)
                tempBoard[i][j] = '-'
